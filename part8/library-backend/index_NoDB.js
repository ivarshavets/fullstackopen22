...
// resolver functions return normal objects
const resolvers = {
  Query: {
    ...
    allBooks: (_root, args) => {
      ...
      if (args.genre) {
        booksList = booksList.filter(({ genres }) => genres.includes(args.genre))
      }

      return booksList
    },
    allAuthors: () => authors,
    allAuthorsFiltered: (_root, args) => {
      if(!args.born) {
        return authors
      }

      const byBirthFilter = (author) =>
        args.born === 'YES' ? author.born : !author.born
      return authors.filter(byBirthFilter)
    }
  },
  Mutation: {
    addBook: (_root, args) => {
